...
//! Full DSv4 MLA attention composite on CUDA vs CPU. pos=0 → RoPE is
...
#[test]
fn dsv4_mla_attention_on_cuda_matches_cpu() {
    let Some(dev) = CudaDevice::create().expect("metal init") else { eprintln!("no CUDA — skip"); return; };
    ...
    eprintln!("DSv4 MLA attention on CUDA vs CPU: max|Δ|={e:.3e}");
    assert!(e <= 5e-3, "mla mismatch: {e:.3e}");
    eprintln!("✅ DSv4 MLA attention composite runs on CUDA through the shared op layer, matches CPU.");
}
