...
fn whisper_base_stt_vs_hf() {
    ...
    eprintln!("Whisper-base STT on CUDA: argmax = {argmax} (HF = 50362)");
    assert_eq!(argmax, 50362, "Whisper-base STT argmax != HF 50362");
    eprintln!("✅ Full real Whisper-base encoder→decoder STT matches HF on the shared engine (GB10 sm_121) — cross-attention path verified.");
}
