...
fn gptneo_125m_full_forward_vs_hf() {
    ...
    let top3 = &idx[..3];
    eprintln!("GPT-Neo-125M full forward on CUDA: top3 = {top3:?} (HF = [28, 59, 91])");
    assert_eq!(top3, &[28usize, 59, 91], "GPT-Neo top-3 != HF");
    eprintln!("✅ Full real GPT-Neo-125M forward matches HF on the shared engine (GB10 sm_121) — learned-pos + LayerNorm + separate-qkv path verified.");
}
