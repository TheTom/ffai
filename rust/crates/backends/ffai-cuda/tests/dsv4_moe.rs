...
#[test]
fn dsv4_moe_on_cuda_matches_cpu() {
    ...
    eprintln!("DSv4 MoE on CUDA vs CPU: max|Δ|={e:.3e} (top {top:?})");
    assert!(e <= 5e-3, "dsv4 moe mismatch: {e:.3e}");
    eprintln!("✅ DSv4 MoE feed-forward runs on CUDA through the shared op layer, matches CPU.");
}
