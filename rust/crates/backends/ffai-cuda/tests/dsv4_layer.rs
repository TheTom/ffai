...
#[test]
fn dsv4_attn_subblock_on_cuda_matches_cpu(){
    ...
    eprintln!("DSv4 attn subblock (mHC⊗MLA) on CUDA vs CPU: max|Δ|={e:.3e}");
    assert!(e<=5e-3,"subblock mismatch: {e:.3e}");
    eprintln!("✅ Full DSv4 attention layer unit runs on CUDA through the shared op layer, matches CPU.");
}
