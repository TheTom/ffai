...
fn olmo2_1b_full_forward_vs_hf() {
    ...
    let top3 = &idx[..3];
    eprintln!("OLMo-2-1B full forward on CUDA: top3 = {top3:?} (HF = [198, 8, 13])");
    assert_eq!(top3, &[198usize, 8, 13], "OLMo-2 top-3 != HF");
    eprintln!("✅ Full real OLMo-2-1B forward matches HF on the shared engine (GB10 sm_121) — post-norm + QK-norm path verified.");
}
