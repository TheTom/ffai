...
fn falcon_h1_0_5b_full_forward_vs_hf() {
    ...
    let top3 = &idx[..3];
    eprintln!("Falcon-H1-0.5B full forward on CUDA: top3 = {top3:?} (HF = [593, 531, 587])");
    assert_eq!(top3, &[593usize, 531, 587], "Falcon-H1 top-3 != HF");
    eprintln!("✅ Full real Falcon-H1-0.5B forward matches HF on the shared engine (GB10 sm_121) — hybrid Mamba2+attention path verified.");
}
