...
//! Causal conv1d step (conv1d_causal_step) on Metal vs CPU — the short-conv
...
#[test]
fn conv1d_causal_step_on_metal_matches_cpu(){
    ...
    eprintln!("conv1d_causal_step on Metal vs CPU: y max|Δ|={ey:.3e}  state max|Δ|={es:.3e}");
    assert!(ey<=1e-4 && es<=1e-4, "conv1d mismatch y={ey:.3e} state={es:.3e}");
    eprintln!("✅ Causal conv1d step runs on Apple GPU through the shared op layer, matches CPU.");
}
