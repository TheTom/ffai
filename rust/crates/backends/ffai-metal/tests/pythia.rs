...
fn pythia_160m_full_forward_vs_hf() {
    ...
    eprintln!("Pythia-160m full forward on Metal: argmax = {argmax} (HF = 285)");
    assert_eq!(argmax, 285, "Pythia argmax != HF 285");
    eprintln!("✅ Full real Pythia-160m (GPT-NeoX) forward matches HF on the shared engine (Apple GPU) — parallel-residual path verified.");
}
