...
fn phi_1_5_full_forward_vs_hf() {
    ...
    let top3 = &idx[..3];
    eprintln!("Phi-1.5 full forward on Metal: top3 = {top3:?} (HF = [11, 13, 546])");
    assert_eq!(top3, &[11usize, 13, 546], "Phi top-3 != HF");
    eprintln!("✅ Full real Phi-1.5 forward matches HF on the shared engine (Apple GPU) — parallel single-norm path verified.");
}
