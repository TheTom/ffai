...
fn gemma2_2b_full_forward_vs_hf() {
    ...
    let top3 = &idx[..3];
    eprintln!("Gemma-2-2b full forward on Metal: top3 = {top3:?} (HF = [9707, 235265, 110])");
    assert_eq!(top3, &[9707usize, 235265, 110], "Gemma top-3 != HF");
    eprintln!("✅ Full real Gemma-2-2b forward matches HF on the shared engine (Apple GPU) — geGLU + RMSNorm(1+w) + 4-norm path verified.");
}
