...
fn stablelm2_1_6b_full_forward_vs_hf() {
    ...
    let top3 = &idx[..3];
    eprintln!("StableLM-2-1.6B full forward on Metal: top3 = {top3:?} (HF = [341, 11, 280])");
    assert_eq!(top3, &[341usize, 11, 280], "StableLM-2 top-3 != HF");
    eprintln!("✅ Full real StableLM-2-1.6B forward matches HF on the shared engine (Apple GPU) — LayerNorm+bias / qkv-bias / partial-rotary path verified.");
}
