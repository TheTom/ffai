...
/// Resident-Q8 decode path on a **k-quant** model: Qwen2.5-7B-Instruct
/// **Q4_K_M** (2-part split GGUF). `open_q8` repacks the Q4_K/Q6_K super-blocks
/// to resident Q8 (dequant → re-quant per-32-block) and decodes through
/// `gemv_q8` — the fast GGUF path all backends use. This is the proof the
/// resident-Q8 + k-quant combo works end-to-end and is far faster than the f32
/// dequant-to-upload path. Runs the SAME prompt through both `open` (f32) and
/// `open_q8` (resident-Q8) and reports decode tok/s + token-stream agreement.
/// Set `QWEN25_7B_GGUF` to part 1 of the split (`*-00001-of-00002.gguf`).
#[test]
fn qwen25_7b_q4km_gguf_q8_resident_decode_on_metal() {
    let Some(dev) = MetalDevice::create().expect("metal init") else {
        eprintln!("no Metal device — skipping");
        return;
    };
    ...
    // ── f32 path (baseline; the slow dequant-to-upload path) ──────────────
    let t_load = Instant::now();
    let m_f32 = GgufModel::open(d, &path, 256).expect("load f32");
    eprintln!("f32 model loaded in {:.2}s", t_load.elapsed().as_secs_f32());
    let _ = m_f32.step(d, prompt[0], 0).expect("f32 warmup");
    let t = Instant::now();
    let mut decode_steps_f32 = 0usize;
    let out_f32 = generate(&prompt, &stop, &Sampling::Greedy, 0, |token, pos| {
    ...
    // ── Q8 path (k-quant super-blocks repacked → resident Q8) ─────────────
    let t_load = Instant::now();
    let m_q8 = GgufModel::open_q8(d, &path, 256).expect("load q8 (k-quant repack)");
    eprintln!("q8 model loaded in {:.2}s", t_load.elapsed().as_secs_f32());
    let _ = m_q8.step(d, prompt[0], 0).expect("q8 warmup");
    let t = Instant::now();
    let mut decode_steps_q8 = 0usize;
    let out_q8 = generate(&prompt, &stop, &Sampling::Greedy, 0, |token, pos| {
    ...
    eprintln!("─── Qwen2.5-7B-Q4_K_M decode tok/s (decode-only, greedy) ───");
    eprintln!("  f32 gemv : {f32_tps:6.2} tok/s  ({decode_steps_f32} steps / {f32_secs:.2}s)");
    eprintln!("  Q8  gemv : {q8_tps:6.2} tok/s  ({decode_steps_q8} steps / {q8_secs:.2}s)");
    eprintln!("  speedup  : {:.2}×", q8_tps / f32_tps.max(1e-6));
    eprintln!("f32 OUTPUT: {f32_text:?}");
    ...
    assert!(
        q8_text.contains("Paris"),
        "Q8 (k-quant→Q8) path lost coherence, got {q8_text:?}"
    );
    eprintln!("✅ Qwen2.5-7B-Q4_K_M resident-Q8 decode coherent on Apple GPU (k-quant repack → gemv_q8).");
}
