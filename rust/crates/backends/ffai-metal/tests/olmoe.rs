...
fn olmoe_1b7b_full_forward_vs_hf() {
    ...
    eprintln!("OLMoE-1B-7B full forward on Metal: argmax = {argmax} (HF = 310)");
    assert_eq!(argmax, 310, "OLMoE argmax != HF 310");
    eprintln!("✅ Full real OLMoE-1B-7B (64-expert MoE) forward matches HF on the shared engine (Apple GPU).");
}
