...
fn llama_causal_prefill_rope_vs_hf() {
    ...
    eprintln!("Llama (SmolVLM-text) RoPE causal prefill (seq={seq}) on Metal: top3 = {:?} (HF = [12642, 4052, 216])", &idx[..3]);
    assert_eq!(&idx[..3], &[12642usize, 4052, 216], "Llama prefill top-3 != HF");
    eprintln!("✅ Llama RoPE-at-position causal prefill matches HF on the shared engine (Apple GPU) — full prefill path verified.");
}
