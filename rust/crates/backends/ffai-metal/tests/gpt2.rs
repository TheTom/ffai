...
fn gpt2_124m_full_forward_vs_hf() {
    ...
    eprintln!("GPT-2-124M full forward on Metal: argmax = {argmax} (HF = 198)");
    assert_eq!(argmax, 198, "GPT-2 argmax != HF 198");
    eprintln!("✅ Full real GPT-2 forward matches HF on the shared engine (Apple GPU) — LayerNorm-LLM path verified.");
}
