...
fn gpt2_causal_prefill_vs_hf() {
    ...
    eprintln!("GPT-2 causal prefill (seq={seq}) on Metal: top3 = {:?} (HF = [11, 21831, 7586])", &idx[..3]);
    assert_eq!(&idx[..3], &[11usize, 21831, 7586], "GPT-2 prefill top-3 != HF");
    eprintln!("✅ GPT-2 multi-token causal prefill matches HF on the shared engine (Apple GPU) — causal prefill path verified.");
}
