...
fn smolvlm_connector_vs_hf() {
    ...
    eprintln!("SmolVLM connector on Metal: out[0,:5]={:?}", &out[..5]);
    ...
    eprintln!("✅ SmolVLM connector (pixel-shuffle + modality projection) matches HF on the shared engine (Apple GPU) — VLM stitch component verified.");
}
