...
extension Ops {
    ...
    public static func dsv4SdpaDecodeD512Sink(
        ...
        // dispatchThreads → gridSize is in THREADS. One threadgroup (512
        // threads) per q head, so the grid is nQHeads*512 threads wide.
        // (A bare `nQHeads` collapses to a single partial threadgroup with
        // tgid_x=0, computing ONLY head 0 and leaving heads 1..63 zeroed.)
        let gridSize = MTLSize(width: nQHeads * 512, height: 1, depth: 1)
    ...
    public static func dsv4PartialRope(
        ...
        freqScale: Float = 1.0, extFactor: Float = 0.0,
        corrLow: Float = 0.0, corrHigh: Float = 0.0,
        on cmd: MTLCommandBuffer
    ) {
        ...
        case .f32:
            MetalTileKernels.ffai_dsv4_partial_rope_f32(
                ...
                freq_scale: freqScale, ext_factor: extFactor, corr_low: corrLow, corr_high: corrHigh,
                gridSize: gridSize, threadgroupSize: tg, on: cmd)
        case .f16:
    ...
    /// BATCHED partial RoPE over `nTokens` rows in ONE dispatch (token t at
    /// position basePosition+t). qk/out [nTokens, nHeads, headDim]. Replaces
    /// the prefill per-token rope loop (N dispatches → 1).
    public static func dsv4PartialRopeRows(
        qk: Tensor, out: Tensor,
        nHeads: Int, headDim: Int, nNope: Int, nTokens: Int, basePosition: Int,
        thetaBase: Float, inverse: Bool,
        freqScale: Float = 1.0, extFactor: Float = 0.0,
        corrLow: Float = 0.0, corrHigh: Float = 0.0,
        on cmd: MTLCommandBuffer
        ...
        let halfRot = (headDim - nNope) / 2
        let gridSize = MTLSize(width: nHeads, height: halfRot, depth: nTokens)
        let tg = MTLSize(width: 1, height: 1, depth: 1)
        let hd = UInt32(headDim); let nNopeU = UInt32(nNope); let halfRotU = UInt32(halfRot)
        let nHeadsU = UInt32(nHeads); let baseU = UInt32(basePosition)
        let invFlag: UInt32 = inverse ? 1 : 0
        switch qk.dtype {
        case .f32:
            MetalTileKernels.ffai_dsv4_partial_rope_rows_f32(
                qk: qk.buffer, qkOffset: qk.offset, out: out.buffer, outOffset: out.offset,
                head_dim: hd, n_nope: nNopeU, half_rot: halfRotU, n_heads: nHeadsU,
                base_position: baseU, theta_base: thetaBase, inverse_flag: invFlag,
                freq_scale: freqScale, ext_factor: extFactor, corr_low: corrLow, corr_high: corrHigh,
                gridSize: gridSize, threadgroupSize: tg, on: cmd)
        case .f16:
            MetalTileKernels.ffai_dsv4_partial_rope_rows_f16(
                qk: qk.buffer, qkOffset: qk.offset, out: out.buffer, outOffset: out.offset,
                head_dim: hd, n_nope: nNopeU, half_rot: halfRotU, n_heads: nHeadsU,
                base_position: baseU, theta_base: thetaBase, inverse_flag: invFlag,
                freq_scale: freqScale, ext_factor: extFactor, corr_low: corrLow, corr_high: corrHigh,
                gridSize: gridSize, threadgroupSize: tg, on: cmd)
        case .bf16:
            MetalTileKernels.ffai_dsv4_partial_rope_rows_bf16(
                qk: qk.buffer, qkOffset: qk.offset, out: out.buffer, outOffset: out.offset,
                head_dim: hd, n_nope: nNopeU, half_rot: halfRotU, n_heads: nHeadsU,
                base_position: baseU, theta_base: thetaBase, inverse_flag: invFlag,
                freq_scale: freqScale, ext_factor: extFactor, corr_low: corrLow, corr_high: corrHigh,
                gridSize: gridSize, threadgroupSize: tg, on: cmd)
        default:
            fatalError("dsv4PartialRopeRows: unsupported qk dtype \(qk.dtype)")
        }
    }
    ...
    public static func dsv4CsaSdpaDecode(
        ...
        let result = out ?? Tensor.empty(shape: [nQHeads, headDim], dtype: outDtype)
        // dispatchThreads → gridSize in THREADS: nQHeads*512 (one 512-thread
        // threadgroup per q head). A bare nQHeads computes only head 0.
        let gridSize = MTLSize(width: nQHeads * 512, height: 1, depth: 1)
        let tg = MTLSize(width: 512, height: 1, depth: 1)
        let hd = UInt32(headDim)
