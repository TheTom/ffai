...
//
// DeepSeek V4 family root — DeepSeek V4 line (DeepSeek-V4-Flash,
// DeepSeek-V4-Pro).
//
...
// scope spans 4-5 distinct PRs).
//
// The GGUF path is SEPARATE: `GGUFTensorBundle` (in `Loader/GGUF/`) is
// a parallel DSv4 loader reached via `loadModelFromGGUF`, not the
// safetensors `SafeTensorsBundle` flow. It does not yet mirror
// `SafeTensorsBundle`'s public surface, so the two are not
// interchangeable in the dispatcher — unifying them behind a shared
// `TensorBundle` protocol is future work. The GGUF reader ships
// alongside the family namespace so the load-from-GGUF infrastructure
// goes in together.
