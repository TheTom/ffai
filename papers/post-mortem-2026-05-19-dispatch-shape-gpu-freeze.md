...
- 📋 **Deferred:** kernel-side OOB guards in `mlx/rms_norm.rs` (already-filed metaltile follow-up to make the kernel itself bounds-safe for `tid * 4 + 3 >= n`).

The first six are the prevention. The remaining two are belt-and-braces; if they were already in place, the bug would have surfaced as a metaltile assertion before reaching the GPU.
